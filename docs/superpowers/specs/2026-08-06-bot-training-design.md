...
version check. A committed fixture (`bots/src/fixtures/random-init.json`,
generated by `training/scripts/make_fixture.py`) holds small seeded-random
weights for smoke tests. It lives in `bots/` rather than `training/` because
the TS registry imports it directly to register the `nn-random` bot.
