...
//!
//! One generation's games are driven by a single `Pool` in lockstep: every ply,
//! all live games are encoded in parallel (rayon), forwarded in one grouped
//! matmul (weights read once per present genome), and applied in parallel. One
//! sync point per ply — no channels, no mutex, no worker threads, no per-forward
//! weight re-slicing.
...
pub struct EvalInputs<'a> {
    ...
    pub device: &'a Device,
    pub n_workers: usize, // unused: rayon drives encode/apply; kept for API compat
}
...
pub fn evaluate_generation(inputs: &EvalInputs<'_>, elo: &mut EloTracker) {
    ...
    let results = rollout(&roster, arch, game_seeds, &meta, *max_hands, device);
