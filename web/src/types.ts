 ...
 *
 * `ts-rs` generation was closed as not relevant, so these mirror the serde
 * derives by hand. The literals they describe are pinned on the Rust side by
 * `engine/crates/canastra-engine/tests/boundary.rs` — that file is the
 ...
 *
 * Only the bot uses this, to guess whether §6's opening minimum is within
