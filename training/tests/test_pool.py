...
def test_pool_plays_full_matches_to_completion() -> None:
    # §6.1 (a failed opening steps the bar up one tier) makes a uniform-random
    # policy pathological: it repeatedly dead-ends openings, raising its own bar
    # to 120/240, after which it almost never opens and scores −300 a hand. The
    # loop still finishes, but only after hundreds of thousands of plies. So the
    # completion driver is the greedy policy below, which actually opens — the
    # point of this test is the loop's batched crossings, not policy strength.
